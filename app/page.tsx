    ...
    <div className="groupay-root" ref={rootRef}>
      ...
      <nav className="fixed top-0 left-0 z-50 w-full pointer-events-none">
        <div className="pointer-events-auto mx-4 mt-4 rounded-2xl border border-white/20 bg-white/80 backdrop-blur-lg shadow-sm">
          <div className="flex items-center justify-between px-6 h-15">
            <Link href="/" className={`${soraClass} font-bold text-forest flex items-center gap-2 text-xl`}>
            ...
            <div className="hidden md:flex items-center gap-x-6">
              <Link href="/#how-it-works" className="text-sm font-medium text-ink-mid hover:text-forest transition-colors">How it Works</Link>
              <Link href="/#features" className="text-sm font-medium text-ink-mid hover:text-forest transition-colors">Features</Link>
            </div>
            <div className="flex items-center gap-3">
              <a href={data?.user ? "/dashboard" : "/auth/sign-in"} className="text-[14px] font-semibold text-forest hover:text-teal transition-colors py-2">
                {data?.user ? data.user.name : "Log In"}
              </a>
              <a href="/auth/sign-up" className="bg-green hover:bg-greener text-white inline-flex items-center gap-2 py-2.5 px-5 rounded-xl font-semibold text-sm transition-all shadow-md shadow-green/25 hover:shadow-lg hover:shadow-green/35 hover:-translate-y-px">
                Get Started →
              </a>
            </div>
          </div>
      ...
      >
        <div className="absolute inset-0 z-0 pointer-events-none">
          <div className="blob1 absolute rounded-full opacity-30 bg-aqua blur-3xl" style={{ width: 480, height: 480, top: -120, right: -120 }} />
          <div className="blob2 absolute rounded-full opacity-20 bg-green blur-3xl" style={{ width: 360, height: 360, bottom: -60, left: -60 }} />
          <div className="blob3 absolute rounded-full opacity-20 bg-teal blur-3xl" style={{ width: 240, height: 240, bottom: 120, right: "18%" }} />
          <div className="blob4 absolute rounded-full opacity-15 bg-aqua blur-2xl" style={{ width: 180, height: 180, top: "40%", left: "8%" }} />
          <div className="absolute inset-0 opacity-[0.025]" style={{ backgroundImage: "linear-gradient(#0d3d2a 1px, transparent 1px), linear-gradient(90deg, #0d3d2a 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
        </div>
        ...
        <div className="hero-content relative z-10 flex flex-col items-center max-w-5xl mx-auto">
          ...
          <p className="hero-subtitle opacity-0 text-lg text-ink-mid max-w-lg mb-10 leading-relaxed">
            GrouPay makes it effortless to split expenses with anyone, track who owes what, and pool money toward shared goals — all in one beautiful place.
          </p>

          <div className="hero-actions opacity-0 flex flex-wrap justify-center gap-4 mb-14">
            <a href="/auth/sign-up" className="inline-flex items-center gap-2 bg-green hover:bg-greener text-white font-semibold px-8 py-4 rounded-full shadow-lg shadow-green/30 hover:shadow-xl hover:shadow-green/40 hover:-translate-y-0.5 transition-all duration-200">
              Start for free
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
              </svg>
            </a>
            <a href="#how-it-works" className="inline-flex items-center gap-2 text-forest border border-forest/25 bg-white/60 hover:bg-white font-semibold px-8 py-4 rounded-full backdrop-blur-sm transition-all duration-200 hover:shadow-md">
              See how it works
            </a>
          </div>
            ...
            {[
              ...
              <div key={i} className="stat opacity-0 text-center">
                <div className={`${soraClass} text-[36px] font-bold text-forest leading-none flex items-end justify-center gap-0.5`}>
                  ...
                  <span className="stat-count" data-target={num} data-decimal={decimal ? "true" : "false"}>{num}</span>
        ...
        <div className="w-full flex justify-center px-2">
          ...
          >
            <div className="absolute inset-0 rounded-2xl pointer-events-none" style={{ boxShadow: "inset 0 1px 0 rgba(255,255,255,0.9), inset 0 -1px 0 rgba(0,0,0,0.04)" }} />

            <div className="mockup-header flex items-center justify-between mb-5">
              <div className={`${soraClass} font-bold text-forest text-lg`}>🏖️ Zanzibar Trip — June 2026</div>
              <span className="py-1.5 px-3.5 rounded-full text-xs font-bold tracking-wide uppercase bg-teal/10 text-teal border border-teal/20">3 of 6 settled</span>
            </div>

            <div className="expense-list flex flex-col gap-3 mb-5">
              {[
                {
                  icon: <AirplaneIcon className="h-4 w-4 text-teal" />, iconBg: "bg-teal/15",
                  name: "Group flights", meta: "Paid by Chidi · 6 people · Apr 12",
                  amount: "₦285,000", share: "Your share: ₦47,500",
                  badge: "You paid ✓", badgeBg: "bg-green/15 text-green border border-green/20",
                },
                {
                  icon: <HouseIcon weight="duotone" className="h-4 w-4 text-forest" />, iconBg: "bg-green/20",
                  name: "Beachfront villa (4 nights)", meta: "Paid by Amara · 6 people · Apr 14",
                  amount: "₦360,000", share: "Your share: ₦60,000",
                  badge: "Pending", badgeBg: "bg-amber-400/15 text-amber-700 border border-amber-300/30",
                },
                {
                  icon: <ForkKnifeIcon className="h-4 w-4 text-rose-500" />, iconBg: "bg-rose-100",
                  name: "Rooftop Dinner", meta: "Paid by Emeka · 6 people · Apr 15",
                  amount: "₦96,000", share: "Your share: ₦16,000",
                  badge: "Pending", badgeBg: "bg-amber-400/15 text-amber-700 border border-amber-300/30",
                },
              ].map((row, i) => (
                <div key={i} className="expense-row group flex items-center gap-4 bg-white border border-card-border rounded-xl p-4 hover:border-teal/30 hover:shadow-md transition-all duration-200 cursor-default">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${row.iconBg} group-hover:scale-110 transition-transform duration-200`}>{row.icon}</div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span className={`inline-block mt-1 text-[11px] font-semibold rounded-full py-0.5 px-2.5 ${row.badgeBg}`}>{row.badge}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section id="how-it-works" className="py-28 px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="section-reveal-group mb-14">
            <div className="section-reveal opacity-0 text-xs uppercase text-teal tracking-widest font-bold mb-3">Simple by design</div>
            <h2 className={`section-reveal opacity-0 ${soraClass} text-4xl font-bold text-forest mb-4`}>Up and running in seconds</h2>
            <p className="section-reveal opacity-0 text-ink-mid max-w-md text-base leading-relaxed">
              No spreadsheets. No awkward reminders. Just GrouPay doing the maths while you enjoy the moment.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {[
              { num: "01", icon: "👥", title: "Create a group", body: "Name your group, set a currency, and invite members by email, phone, or a shareable link. They're in with one tap.", accent: "from-teal/10 to-aqua/5" },
              { num: "02", icon: "🧾", title: "Add expenses", body: "Log any bill — equally split or custom. Snap the receipt for your records. Everyone sees their share instantly.", accent: "from-green/10 to-teal/5" },
              { num: "03", icon: "📡", title: "Track & remind", body: "Watch the live balance dashboard. GrouPay auto-calculates who owes what, and sends gentle nudges so you don't have to.", accent: "from-aqua/10 to-green/5" },
              { num: "04", icon: "⚡", title: "Settle up", body: "Our debt-simplification algorithm minimises the number of transfers. Confirm payment and everyone's balance resets clean.", accent: "from-teal/10 to-green/5" },
            ].map((step, i) => (
              <div key={i} className={`step-card opacity-0 group relative bg-linear-to-br ${step.accent} border border-card-border p-8 rounded-2xl hover:border-teal/40 hover:shadow-lg transition-all duration-300 overflow-hidden`}>
                <div className={`absolute -top-3 -right-1 ${soraClass} text-[80px] font-black text-forest/5 leading-none select-none pointer-events-none`}>{step.num}</div>
                <div className="text-3xl mb-4">{step.icon}</div>
                <div className={`${soraClass} text-xs font-bold text-teal tracking-wider uppercase mb-2`}>Step {step.num}</div>
      ...
      <section id="features" className="relative py-28 px-6 bg-forest text-white overflow-hidden">
        <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: "radial-gradient(circle, #49c635 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="section-reveal-group mb-14">
            <div className="section-reveal opacity-0 text-xs uppercase tracking-widest font-bold text-teal mb-3">Everything you need</div>
            <h2 className={`section-reveal opacity-0 ${soraClass} text-4xl font-bold mb-4`}>Built for real life</h2>
            <p className="section-reveal opacity-0 text-white/60 max-w-md text-base leading-relaxed">
              From spontaneous dinners to year-long savings goals, GrouPay handles every kind of shared money moment.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="feature-card opacity-0 group relative bg-white/5 border border-white/10 p-7 rounded-2xl hover:bg-white/10 hover:border-teal/40 transition-all duration-300 overflow-hidden">
      ...
      <section className="cta-banner relative py-24 px-6 text-center overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: "linear-gradient(#0d3d2a 1px, transparent 1px), linear-gradient(90deg, #0d3d2a 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
        <div className="cta-inner opacity-0 relative z-10 max-w-2xl mx-auto">
          ...
          <h2 className={`${soraClass} text-[clamp(32px,6vw,52px)] font-bold text-forest mb-4 leading-tight`}>Ready to end the awkwardness?</h2>
          <p className="text-ink-mid mb-10 text-lg">Join 2.4 million people who've made shared money simple.</p>
          <Link href="/auth/sign-up" className="inline-flex items-center gap-2 bg-green hover:bg-greener text-white font-semibold text-lg py-4 px-10 rounded-full shadow-xl shadow-green/30 hover:shadow-2xl hover:shadow-green/40 hover:-translate-y-1 transition-all duration-200">
        ...
        <div className="max-w-6xl mx-auto px-6 pt-14 pb-8">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 pb-10 border-b border-white/10">
            <div className="col-span-2">
              ...
              <p className="text-sm leading-relaxed max-w-xs">Making shared spending fair, transparent, and friction-free for groups everywhere.</p>
            </div>
            {[
              { heading: "Product", links: ["Bill Splitting", "Group Goals", "Dashboard", "Integrations", "Mobile App"] },
              ...
              <div key={col.heading}>
                <p className={`${soraClass} text-xs font-bold text-white tracking-widest uppercase mb-4`}>{col.heading}</p>
                {col.links.map((l) => (
                  <a key={l} href="#" className="block text-sm text-white/50 hover:text-teal transition-colors mb-2.5">{l}</a>
                ))}
              </div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6">
            ...
            <div className="flex items-center gap-5">
              {["Twitter", "Instagram", "LinkedIn"].map((s) => (
                <a key={s} href="#" className="text-sm text-white/40 hover:text-aqua transition-colors">{s}</a>
              ))}
            </div>
          </div>
        </div>
