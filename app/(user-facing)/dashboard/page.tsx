...
import { PlusIcon, ArrowRightIcon, ArrowUpIcon, ArrowDownIcon, ArrowsLeftRightIcon } from "@phosphor-icons/react";
...
export default function DashboardPage() {
  ...
  const [prompter, updatePrompter] = useState<"add" | "withdraw" | "transfer">("add");
  ...
  async function eleba() {
    ...
    const results = await Promise.all(fetchedClustIds.map((clust: any) => fetchClust(clust)));
    return results;
  }

  const { data: clusterResponse, isLoading, isSuccess } = useQuery({
  ...
  useGSAP(
    () => {
      const ctx = gsap.context(() => {
        ...
        .fromTo(".dash-quick-actions .action-btn",
          { y: 20, opacity: 0, scale: 0.95 },
          { y: 0, opacity: 1, scale: 1, stagger: 0.08, duration: 0.4 },
          "-=0.3"
        )
        .fromTo(".dash-onboarding",
          { y: 20, opacity: 0 },
          { y: 0, opacity: 1, duration: 0.5 },
          "-=0.2"
        );
        ...
        gsap.utils.toArray<HTMLElement>(".cluster-card-item").forEach((card) => {
          gsap.fromTo(card,
            { x: 30, opacity: 0 },
            {
        ...
        gsap.fromTo(".fab-btn",
          ...
          { scale: 1, opacity: 1, rotate: 0, duration: 0.6, ease: "back.out(1.7)", delay: 0.8 }
        );
    ...
    <div ref={rootRef} className="min-h-screen bg-[#f7faf7] pb-24">
      ...
      <div className="bg-white border-b border-[#e8efe8] px-6 py-5">
        ...
        <div className="dash-greeting opacity-0 flex items-center justify-between mb-5">
          <div>
            ...
            <h1 className={`${soraClass} text-2xl font-bold text-forest leading-tight`}>
          ...
          {[
            {
              ...
              type: "add" as const,
              Icon: ArrowDownIcon,
              color: "bg-green/10 text-green hover:bg-green hover:text-white border border-green/20",
            },
            {
              label: "Withdraw",
              type: "withdraw" as const,
              Icon: ArrowUpIcon,
              color: "bg-teal/10 text-teal hover:bg-teal hover:text-white border border-teal/20",
            },
            {
              label: "Transfer",
              type: "transfer" as const,
              Icon: ArrowsLeftRightIcon,
              color: "bg-forest/10 text-forest hover:bg-forest hover:text-white border border-forest/20",
            },
          ].map(({ label, type, Icon, color }) => (
            <button
              key={type}
              onClick={() => { updateModalState(true); updatePrompter(type); }}
      ...
      <div className="px-6 mt-6">
        <div className="flex items-center justify-between mb-3">
          <p className={`section-heading opacity-0 ${soraClass} font-bold text-forest text-lg`}>
            Your Clusters
          </p>
          <Link
            href="/clusters"
            className="section-heading opacity-0 flex items-center gap-1.5 text-sm font-semibold text-teal hover:text-forest transition-colors"
          >
            View all
        ...
        <div className="flex gap-4 overflow-x-auto pb-3 -mx-1 px-1 scrollbar-hide">
          ...
          {isSuccess && clusterResponse.map((cluster) => (
            <div key={cluster.id} className="cluster-card-item opacity-0 shrink-0">
