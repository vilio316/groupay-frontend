...
export function MemberCard({ member }: { member: ClusterMember }) {
  ...
  return (
    <div
      className={`${isYou ? "bg-green/40" : ""} flex gap-x-4 w-4/5 items-center hover:bg-aqua/20 hover:scale-105 transition-all rounded-xl p-2 my-3 group`}
    >
      <div className="p-1">
        <img
      ...
      <div className="w-[80%]">
        <p>
          {member.user.name === data?.user.name ? "You" : member?.user.name}
        </p>
